...
# 任务系统函数扩展指南 (自动注册机制)
# =============================================================================
# 新增任务函数只需在 init_task_registry() 中添加一行！
#
# 位置：约第 66 行
#
# 格式：
#     "函数名": {"func": 函数对象, "description": "中文描述"},
#
# 示例：新增 hello_world 函数
#   (1) 在 init_task_registry() 中添加：
#       "hello_world": {"func": hello_world, "description": "测试函数"},
#   (2) 确保 import已 该函数
#
# 进阶：使用装饰器自动注册（推荐）
...
from models.network.uac import init_white
from models.daily.inout import check_inout_days
...
def init_task_registry():
    ...
    TASK_REGISTRY.update({
        "send_text": {"func": send_text, "description": "发送文本"},
        "gk_countdown": {"func": gk_countdown, "description": "高考倒计时"},
        "today_teachers": {"func": today_teachers, "description": "今日教师"},
        "watching_parking": {"func": watching_parking, "description": "监控停车场"},
        "random_daily_task": {"func": task_scheduler.random_daily_task, "description": "随机每日任务"},
        "refresh_cache_data": {"func": refresh_cache_data, "description": "刷新缓存"},
        "create_month_dir": {"func": create_month_dir, "description": "创建月份目录"},
        "clear_temp_file": {"func": clear_temp_file, "description": "清理临时文件"},
        "init_white": {"func": init_white, "description": "初始化白名单"},
        "check_inout_days": {"func": check_inout_days, "description": "检查出入记录"},
        "analyze_log_file": {"func": analyze_log_file, "description": "分析日志文件"},
    })
...
class Task:
    ...
    def add_job(self, func, trigger, *args, **kwargs):
        job = self.scheduler.add_job(func, trigger, misfire_grace_time=60, *args, **kwargs)
        if len(args) > 0:
    ...
    def get_available_funcs(self):
        ...
        :return: 函数名和描述的列表
        """
        return [{"value": k, "label": f"{k} - {v['description']}"} for k, v in TASK_REGISTRY.items()]
