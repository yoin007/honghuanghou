...
def push_qrcode():
    ...
    try:
        # 获取GitHub个人访问令牌
        token = Config().get_config("git_token", "token.yaml")
        # 登录GitHub
        ...
        # 获取群号
        qrcode_git = Config().get_config("qrcode_git", "message.yaml")
        if not qrcode_git:
...
async def change_roomid(record):
    ...
    if name in qrcode_git:
        ...
        if Config().modify_config("qrcode_git", qrcode_git, "message.yaml"):
