...
class Application:
    def __init__(self, db_path="databases/application.db"):
        ...
        self.categories = ['普通', '美术', '音乐', '体育', '书法', '舞蹈', '表导演']
        self.yinyue = ['音乐', '声乐', '器乐']
    ...
    def rank_to_score(self, rank, category, year,  fangxiang='', type='综合'):
        """
        专业排名转分数 (带缓存)
        :param rank: 专业排名
        :param category: 专业类别
        ...
        table_name = 'putongfenduan' if category == "普通" else 'yitifenduan'
        # 构建查询语句
        query = f"SELECT 分数 FROM {table_name} WHERE 年份 = ? AND 类型 = ? AND 累计人数 <= ?"
        
        # 对于艺体类专业，添加类型条件
        if category != "普通":
            query += f" AND 类别 = '{type}'"
            if category == '音乐':
                query += f" AND 方向 = '{fangxiang}'"
            if category == '表导演':
                query += f" AND 方向 = '{fangxiang}'"
        ...
        with self as app:
            app.__cursor__.execute(query, (year, category, rank))
            result = app.__cursor__.fetchone()
        
        return result[0] if result else -1

    @lru_cache(maxsize=128)
    def score_to_rank(self, score, category, year,  fangxiang='', type='综合'):
        ...
        table_name = 'putongfenduan' if category == "普通" else 'yitifenduan'
        # 构建查询语句
        query = f"SELECT 累计人数 FROM {table_name} WHERE 年份 = ? AND 类型 = ? AND 分数 <= ?"
        
        # 对于艺术类专业，添加类型条件
        if category != "普通":
            query += f" AND 类别 = '{type}'"
            if category == '音乐' and fangxiang:
                query += f" AND 方向 = '{fangxiang}'"
            if category == '表导演' and fangxiang:
                query += f" AND 方向 = '{fangxiang}'"
        
        query += " ORDER BY 分数 DESC LIMIT 1"
        # print(query, year, category, score)
        with self as app:
            app.__cursor__.execute(query, (year, category, score))
            result = app.__cursor__.fetchone()
        
        return result[0] if result else -1

    def toudang(self, category, zy, year="", yx="", rank=0, fenshu=0, fangxiang='', counts=30):
        """
        查询投档情况
        Args:
            category: 类别
        ...
        table_name = 'putongtoudang' if category == "普通" else 'yititoudang'
        ...
        if rank:
            if category == "普通":
                ...
                score = self.rank_to_score(rank, category, year, fangxiang)
                conditions.append("最低分数 >= ?")
                params.append(score)
        else:
            if fenshu:
                if category == "普通":
                    rank = self.score_to_rank(fenshu, category, year, fangxiang)
                    conditions.append("最低位次 <= ?")
                    params.append(rank)
                else:
                    conditions.append("最低分数 >= ?")
                    params.append(fenshu)
        ...
        if category != "普通":
            columns = [
                "类型", "年份", "批次", "层次", "计划数", "最低分数", "专业", "院校",  
                "方向"
            ]
            df = pd.DataFrame(results, columns=columns).drop(columns=['方向'])
        else:
            columns = [
                "类型", "年份", "批次", "层次", "计划数", "最低位次", "专业", "院校"
            ]
        ...
        if category != "普通":
            ...
            df["位次"] = df.apply(
                lambda row: self.score_to_rank(row["最低分数"], row["类型"], row["年份"], fangxiang),
                axis=1
            )
            ...
            df[f"{self.year}分数(同位次)"] = df.apply(
                lambda row: self.rank_to_score(row["位次"], row["类型"], self.year, fangxiang),
                axis=1
            )
        else:
            # 普通处理
            ...
            df["分数"] = df.apply(
                lambda row: self.rank_to_score(row["最低位次"], row["类型"], row["年份"], fangxiang),
                axis=1
            )
            ...
            new_order = [
                ...
                "计划数", "最低位次", "分数", "层次", f"{self.year}分数(同位次)"
            ]
            # 添加额外信息
            # df["选科要求"] = df.apply(
    ...
    def toudang_range(self, category, year, min_rank, max_rank, fangxiang=''):
        """根据类别、年份和位次范围查询投档情况
        
        Args:
            category: 类别，如"普通"、"美术"等
            year: 年份
            min_rank: 最小位次
            max_rank: 最大位次
...
def calculate_gradient_intervals(
    ...
    # 根据考生类型设置总志愿数
    total_slots = 96 if category == "普通" else 60
    if category == "普通":
    ...
    if category == '艺术类':
        adjustment_factors['专业类型'] = {
            '美术': +0.02 if segment_type == "高分" else -0.01,
...
async def gradient_intervals(record):
    ...
    if len(text) != 4:
        send_text("请输入正确的格式，如：位次区间-普通-均衡-3456", record.roomid)
        return None, None
