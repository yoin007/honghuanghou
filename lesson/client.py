...
class Client:
    def __init__(self) -> None:
        self.config = Config()
        self.base_url = self.config.get_config("base_url", "wechat.yaml")
        self.wxid = self.config.get_config("bot_wxid", "wechat.yaml")
        self.LOG = logging.getLogger("Bot")
        self._token = self.config.get_config("token", "token.yaml")
        self.static_url = self.config.get_config("static_url", "wechat.yaml")
    ...
    def get_group_members(self, roomid):
        """
        获取群成员信息
        """
        all_members = []
        ...
        while current_page <= total_page:
            try:
                response = self._make_request("get_contact_user_info.html", {"chat_room_id": roomid}, { "page": str(current_page)})

                if response:
                    result = json.loads(response)
                    if result.get("success"):
                        page_data = result.get("data", {})
                        members = page_data.get("list", [])
