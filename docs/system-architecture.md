...
截至 2026-05-13，系统权限控制已从“按角色硬编码为主”收口到“数据库动态配置为主”：

- API 是否可调用：`api_permission_config`
- 查看哪些记录：`data_scope_rules`
- 创建时能选择哪些目标对象：`target_scope_rules`
- 编辑、删除、复核、导出等动作边界：`operation_scope_rules`

前端配置入口：

- `/moral/config/api-permission`
...
- `auth-system-blueprint.md`
- `auth-frontend-adaptation-plan.md`
- `unified-permission-optimization-plan.md`
...
| API 权限配置 | ✅ ApiPermission.vue | ✅ CRUD / 审计 / 预览 | ✅ 已完成 |
...
### 权限治理补充 (2026-05-13)
- 统一 API 权限、数据范围、目标范围、动作范围配置模型
- `moral/config/api-permission` 支持范围编辑、预览校验、批量角色、风险巡检与导出
- dashboard 关键接口、趋势接口、任务结转、菜单权限、数据库管理等接口纳入统一配置
- 当前权限巡检 `174 / 0`，后端测试 `608 passed`
