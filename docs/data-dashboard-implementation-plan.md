...
6. **学期隔离**：驾驶舱默认以当前学期（`semester.status = 1`）为统计单位。若 API 提供了 `start_date`/`end_date` 参数，则按日期范围查询；否则所有 `moral_evaluation` 和 `student_daily_record` 查询必须过滤 `semester_id`。
7. **分阶段落地**：先做能复用现有表的统计，再做趋势、快照、缓存和更复杂的分析。
